...
func ShowNewsCom(c *fiber.Ctx) error {
	var data []models.NewsComment
...
func IndexNewsCom(c *fiber.Ctx) error {
	id := c.Params("id")
	var data models.NewsComment
...
func CreateNewsCom(c *fiber.Ctx) error {
	var data models.NewsComment
...
func UpdateNewsCom(c *fiber.Ctx) error {
	...
	if updateData.ID != 0 && updateData.ID != id {
		if err := models.DB.First(&models.NewsComment{}, updateData.ID).Error; err == nil {
...
func DeleteNewsCom(c *fiber.Ctx) error {
	id := c.Params("id")

	if models.DB.Delete(&models.NewsComment{}, id).RowsAffected == 0 {
